...
def get_imagenet_standard_transform1(image_size=384, crop_pct=0.875,
                                    interpolation=torchvision.transforms.InterpolationMode.BICUBIC,
                                    mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD):
...
def test(models, test_loader):
    ...
    img_names = []
    all_id_preds = []       # Store class preds
    all_osr_preds = []      # Stores OSR preds
    softmax_sum = None
    # transform_1, transform_2, transform_3 = get_imagenet_standard_transform1()
    transform_1, transform_3 = get_imagenet_standard_transform1()
    ...
    for batch_idx, (images, _, _, img_name) in enumerate(tqdm(test_loader)):
        ...
        for model in models:
            ...
            for transform in [transform_1, transform_3]:# transform_1, transform_2, transform_3
                augmented_images = []
                for image in images:
                    pil_image = F.to_pil_image(image.cpu())  
                    augmented_image = transform(pil_image)   
                    augmented_images.append(augmented_image)
                augmented_images = torch.stack(augmented_images).cuda()  
            ...
            # Softmax
            sftmax = torch.nn.functional.softmax(logits_avg, dim=-1)
            if i==0:
            ...
            all_id_preds.extend(sftmax.argmax(dim=-1).cpu().numpy())
            all_osr_preds.extend(sftmax.max(dim=-1)[0].cpu().numpy())
...
def main():
    ...
    for k, v, in datasets.items():
        if v is not None:
            dataloaders[k] = DataLoader(v, batch_size=64, shuffle=False, sampler=None, num_workers=8)
    ...
    # Get preds
    image_names_know, id_preds, osr_preds_id_samples = test(models, dataloaders['test_known'])
    image_names_unknow, id_preds_, osr_preds_osr_samples = test(models, dataloaders['test_unknown'])
    print(len(id_preds))
    print(len(id_preds_))
    # csv
    ...
    # osr_preds_combined = np.where(np.array(osr_preds_combined) >= 0.1, 1, 0).tolist()
    ...
    df.to_csv(f'./out-deit-384-tta/output-{args.data}.csv', index=False)
